...
UdpLightSensorReader::~UdpLightSensorReader()
{
    stop();
}
...
void UdpLightSensorReader::start()
{
    ...
    connect(m_udpSocket, &QUdpSocket::readyRead, this, &UdpLightSensorReader::readPendingDatagrams);
    m_running = true;
...
LightSensorData UdpLightSensorReader::parseJson(const QByteArray &jsonData) const
{
    ...
    data.gainIndex = obj["gain"].toInt();  // усиление в виде индекса (например 32)
