...
private:
    /**
     * @brief Преобразует желаемое время экспозиции (мс) в параметры atime и
     * astep датчика AS7341.
     *
     * Датчик AS7341 управляется регистрами ATIME и ASTEP. Время экспозиции T
     * (мс) вычисляется как: T = (atime+1) * (astep+1) * 2.78 / 1000
     *
     * В данной реализации atime фиксирован (значение 10), а astep вычисляется
     * линейно относительно опорной точки, полученной экспериментально: при
     * atime = 10, astep = 20000, измеренное время экспозиции составляет 556 мс.
     * Для удобства масштабирования за базовую принята экспозиция 556 мс
     * (опорное значение refMs = 556), которой соответствуют atime = refAtime =
     * 10, astep = refAstep = 20000.
     *
     * Формула пересчёта:
     *   astep = ( (refAstep + 1) * ms / refMs ) - 1
     *
     * @param ms   Желаемое время экспозиции в миллисекундах (должно быть > 0).
     * @param atime [выход] Значение регистра ATIME (всегда = refAtime = 10).
     * @param astep [выход] Рассчитанное значение регистра ASTEP (обрезается в
     * диапазон 0…65535).
     *
     * Примечание: при очень малых ms (менее ~12.5 мс) astep может стать
     * отрицательным, тогда принудительно устанавливается 0. При больших ms (>
     * ~1638 мс) astep может превысить 65535, тогда он ограничивается
     * максимальным значением.
     */
    void msToAtimeAstep(int ms, int &atime, int &astep) const;
